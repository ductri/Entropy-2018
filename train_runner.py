...
def run(name):
    with tf.Graph().as_default() as gr:
        ...
        tf_writer = tf.summary.FileWriter(logdir=os.path.join(CURRENT_DIR, 'summary', str(name)), graph=gr)
        ...
        with tf.Session().as_default() as sess:
            ...
            _, global_step = sess.run([tf_optimizer, tf_global_step],
                                      feed_dict={
                                          tf_input: np.random.rand(BATCH_SIZE, model_v1.SENTENCE_LENGTH_MAX),
                                          tf_labels: np.random.randint(low=0, high=3, size=(BATCH_SIZE))
                                      })
            if global_step % 10 == 0:
                summary_data = sess.run(tf_all_summary)
